...
module DiceBag
  ...
  class Configuration
    ...
    def self.write_all
      self.write_multiple( File.join(Rails.root, "config/*.erb") )
      self.write_multiple( File.join(Rails.root, "config/initializers/*.erb") )
    end

    def self.write_multiple(template_names)
